...
class Optimizer(Dynamics):
    ...
    def log(self, forces=None):
        if forces is None:
            forces = self.atoms.get_forces()
        # add by Wei Gao
        fmax_atom = sqrt((forces[0:-3] ** 2).sum(axis=1).max())
        fmax_box = sqrt((forces[-3:] ** 2).sum(axis=1).max())
        # 
        ...
        if self.logfile is not None:
            ...
            if self.nsteps == 0:
                # modify by Wei Gao, add output variable names
                args = (" " * len(name), "Step", "Time", "Energy", "fmax", "fmax_atom","fmax_box", "stress_max")
                msg = "%s  %4s %8s %15s %14s % 14s %14s %14s\n" % args
            ...
            # add by Wei Gao, compute stress difference between the given one and real one
            if self.atoms.stress_type == 'cauchy':
                stress_max = ((self.atoms.get_stress(voigt=False)-self.atoms.express)/units.GPa).max()
            elif self.atoms.stress_type == 'PK1':
                stress_max = ((self.atoms.get_stress_pk1()-self.atoms.express)/units.GPa).max()
            elif self.atoms.stress_type == 'PK2':
                stress_max = ((self.atoms.get_stress_pk2()-self.atoms.express)/units.GPa).max()
            
            #
            
            # add output variables
            args = (name, self.nsteps, T[3], T[4], T[5], e, ast, fmax, fmax_atom, fmax_box, stress_max)
            msg = "%s:  %3d %02d:%02d:%02d %15.6f%1s %14.6E %14.6E %14.6E %14.6E\n" % args
            self.logfile.write(msg)
